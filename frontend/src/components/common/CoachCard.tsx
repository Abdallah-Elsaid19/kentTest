...
export function CoachCard({ name, image, bio, linkedIn, imageAlt, titleAs: Title = 'h3' }: CoachCardProps) {
  return (
    <article className="group flex flex-1 flex-col rounded-2xl border border-kbc-purple-100 bg-white p-6 transition duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-[0_18px_45px_rgba(39,14,73,0.1)] motion-reduce:transform-none motion-reduce:transition-none sm:p-7">
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <Title className="text-xl font-semibold text-[var(--color-ink)]">{name}</Title>
        {linkedIn && <a
          href={linkedIn}
          target="_blank"
          rel="noreferrer"
