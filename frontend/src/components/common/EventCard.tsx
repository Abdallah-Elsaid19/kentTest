  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-[#e5e0e8] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(36,16,45,.11)] motion-reduce:transition-none">
      <div className="event-card-media relative h-[195px] shrink-0 overflow-hidden bg-slate-100">
        ...
        {category && <span className="absolute bottom-3.5 left-4 z-[2] rounded-md bg-[#F5C94F] px-2.5 py-2 text-[11px] font-bold uppercase tracking-[.12em] text-[#2c1e2f]">{category}</span>}
      </div>
      <div className={contentSized ? 'flex flex-1 flex-col gap-5 p-[22px]' : "grid min-h-[300px] flex-1 grid-rows-[92px_92px_1fr] p-[22px] sm:min-h-[360px] sm:grid-rows-[92px_132px_1fr] xl:min-h-[300px] xl:grid-rows-[92px_92px_1fr]"}>
        ...
        <div className="mt-auto flex w-full flex-wrap items-center justify-between gap-3 self-end border-t border-[#e7e1e8] pt-4">
          <ArrowLink className="!min-h-10 !p-0 !text-sm !text-[#401B8C] hover:!text-[#2F1468]" to={detailsTo}>{detailsLabel}</ArrowLink>
          {bookingUrl && <a className="events-gold-button inline-flex min-h-10 items-center justify-center rounded-lg border border-[#F5C94F] bg-[#F5C94F] px-4 py-2.5 text-center text-xs font-bold text-[#401B8C] transition-colors hover:text-white" href={bookingUrl} target="_blank" rel="noreferrer">{bookingLabel}</a>}
        </div>
      </div>
    </article>
  );
