...
const learnerHeroImage = "{{cms:employers.pages_employers_components_learner_hero__learner_hero_image.text_001}}";
...
export function LearnerHeroSection() {
  ...
  const cmsValues = cms.resolve({ learnerHeroImage, heroStats, statIcons });
  ...
  return cms.render((
    <section className="kbc-page-hero-offset relative isolate flex min-h-[100svh] flex-col overflow-hidden bg-primary pb-[clamp(16px,3svh,48px)] text-white" aria-labelledby="employer-hero-title">
      <img
        ...
        src={cmsValues.learnerHeroImage}
        alt=""
        aria-hidden="true"
        fetchPriority="high"
        decoding="async"
      ...
      <div className="figma-shell figma-hero__grid relative z-10 !items-start sm:max-xl:!grid-cols-1">
        <div className="figma-hero__copy flex flex-col items-center text-center sm:block sm:max-xl:!mx-auto sm:max-xl:!flex sm:max-xl:!max-w-[760px] sm:max-xl:!flex-col sm:max-xl:!gap-9 sm:max-xl:!text-center xl:!w-[820px] xl:!max-w-[820px] xl:text-left">
          <p className="figma-hero__eyebrow !mx-auto !text-xs !font-bold !leading-5 !tracking-widest !text-[#F5C94F] [@media(max-height:1000px)_and_(max-width:639px)]:!mb-3 [@media(max-height:1000px)_and_(min-width:1280px)]:!mb-3 sm:max-xl:!mb-0 sm:max-xl:!text-[15px] xl:!ml-0 xl:!mr-0">{cms.text("employers.pages_employers_components_learner_hero__learner_hero_section.text_002")}</p>
          <h1 id="employer-hero-title" className="flex flex-col gap-2 !text-[clamp(28px,9vw,48px)] !font-medium !leading-none !tracking-tight !text-white sm:gap-3 sm:max-xl:!text-[clamp(52px,7.5vw,72px)] xl:!text-[72px]">
            <span className="block whitespace-nowrap text-white">{cms.text("employers.pages_employers_components_learner_hero__learner_hero_section.text_003")}</span>
            <span className="block whitespace-nowrap text-[#F5C94F]">{cms.text("employers.pages_employers_components_learner_hero__learner_hero_section.text_004")}</span>
          </h1>
          <p className="!mt-8 max-w-2xl !text-base !leading-relaxed text-white/80 sm:max-xl:!mt-3 sm:max-xl:!text-[27px] sm:max-xl:!leading-[1.6] xl:!mt-10 xl:!text-lg">
            {cms.text("employers.pages_employers_components_learner_hero__learner_hero_section.text_005")}</p>

          <HeroActionButtons
            primary={{ label: cms.text("employers.pages_employers_components_learner_hero__label.text_006"), to: "#workforce-solutions" }}
            secondary={{ label: cms.text("employers.pages_employers_components_learner_hero__label.text_007"), to: "#partner-with-us" }}
          ...
          <dl className="mt-10 grid max-w-xl gap-5 border-t border-white/15 pt-6 text-left [@media(max-height:1000px)_and_(max-width:639px)]:!mt-5 sm:grid-cols-3 sm:max-xl:!mx-auto sm:max-xl:!mt-0 xl:mx-0">
            {cmsValues.heroStats.map((stat, index) => {
              const Icon = cmsValues.statIcons[index];
              return (
