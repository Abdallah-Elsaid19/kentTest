    ...
    <div className="kbc-figma-home overflow-x-clip bg-white font-body text-kbc-purple-950">
      ...
      <article className={sectionClass}>
        <div className="mx-auto w-full max-w-5xl px-5 sm:px-8">
          <nav aria-label="Breadcrumb"><ArrowLink to={newsPath} direction="up-right" className="rounded-sm text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">{newsHero.eyebrow}</ArrowLink></nav>
          ...
          <div className="mx-auto mt-12 max-w-3xl space-y-7 text-base leading-8 text-kbc-dark-700 sm:text-lg">
            {article.body.map((paragraph, index) => <Fragment key={paragraph}>
              ...
              {index === 1 && article.keyPoints?.length && <aside className="rounded-2xl border-l-4 border-kbc-gold-500 bg-kbc-purple-50 p-6 sm:p-8" aria-labelledby="article-key-points">
                <h2 id="article-key-points" className="!text-2xl !font-semibold text-primary-dark">{newsDetail.keyPoints}</h2>
      ...
      <section className={`${sectionClass} !bg-kbc-purple-50`} aria-labelledby="related-news-title">
        <div className={containerClass}>
          <div className="mb-10 flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
            <SectionIntro id="related-news-title" eyebrow={newsDetail.relatedEyebrow} title={newsDetail.relatedTitle} align="left" spaced={false} />
            <NavigationButton to={newsPath} variant="secondary" className="shrink-0">{newsDetail.allAction}</NavigationButton>
          </div>
...
export default function NewsArticlePage() {
  ...
  const article = newsArticles.find(item => item.id === articleSlug);
  if (!article) return <>
    <RouteMeta fallbackTitle={newsDetail.notFoundTitle} seo={{ robots: "noindex,follow" }} />
    <PageHero className="kbc-page-hero-offset" title={newsDetail.notFoundTitle} summary={newsDetail.notFoundDescription} />
    <div className="px-5 py-12 text-center"><NavigationButton to={newsPath}>{newsDetail.backAction}</NavigationButton></div>
  </>;
