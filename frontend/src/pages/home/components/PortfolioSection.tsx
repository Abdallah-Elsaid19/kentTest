      ...
      <div className="figma-shell figma-portfolio__stack">
        ...
        <div className="figma-portfolio__grid !grid-cols-1 md:!grid-cols-2">
          {colleges.map((college) => {
            const isLeadership = /\/college-of-leadership\/?(?:[?#].*)?$/.test(college.href);
            const className = `figma-college-card is-${college.tone} !min-h-[330px] !p-6 sm:!min-h-[350px] sm:!p-8 xl:!min-h-[370px]`;
            const cardContent = <>
              ...
              <span className="figma-college-card__link-label !text-sm !font-semibold !leading-5">{college.cta} {!isLeadership && <ArrowUpRight aria-hidden="true" />}</span>
            </>;
            return isLeadership
              ? <article className={className} key={college.category}>{cardContent}</article>
              : <Link className={className} key={college.category} to={college.href} aria-label={college.cta}>{cardContent}</Link>;
          })}
        </div>
      </div>
    </section>
