...
export function LearnerTrustedSection() {
  ...
  const logos = [...cmsValues.trustedOrganisationLogos, ...cmsValues.trustedOrganisationLogos];
        ...
        <div className="group relative mt-10 overflow-hidden [mask-image:linear-gradient(90deg,transparent,#000_8%,#000_92%,transparent)]" aria-label="Organisations represented by Kent Business College learners">
          <div className="flex w-max animate-marquee items-center group-hover:[animation-play-state:paused] motion-reduce:animate-none">
            {logos.map((logo, index) => (
              <div className="flex h-20 w-36 shrink-0 items-center justify-center px-5 sm:h-24 sm:w-44" key={`${logo.name}-${index}`} aria-hidden={index >= cmsValues.trustedOrganisationLogos.length}>
                <img
                  ...
                  src={logo.image}
                  alt={index < cmsValues.trustedOrganisationLogos.length ? logo.name : ""}
                  loading="lazy"
                  decoding="async"
                />
              </div>
