...
export function EmployerProjectControlsSection() {
  ...
  const cmsValues = cms.resolve({ commercialCapabilities });
  ...
  return cms.render((
    <section className={`${sectionClass} !bg-primary/[.035]`} id="employer-project-controls">
      <div className={`${containerClass} grid gap-12 lg:grid-cols-2 lg:gap-14`}>
        ...
        <div className="grid grid-cols-2 gap-3 max-[500px]:grid-cols-1">{cmsValues.commercialCapabilities.map((capability) => <div className="flex items-center gap-3 rounded-xl border border-[#e8e0ef] bg-white px-5 py-5" key={capability}><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#fff3ca] text-[#8f6b22]"><ArrowUpRight size={16} /></span><strong>{capability}</strong></div>)}</div>
      </div>
    </section>
  ));
}
