...
export function CommercialAccessSection() {
  ...
  const cmsValues = cms.resolve({ commercialAccessOptions, commercialCapabilities, projectControlModules });
    ...
    <section className={sectionClass} id="commercial-access">
      <div className={containerClass}>
        ...
        <div className="grid grid-cols-[1.1fr_repeat(3,.7fr)] gap-[15px] max-[1050px]:grid-cols-3 max-[780px]:grid-cols-1">
          ...
          {cmsValues.commercialAccessOptions.map(({ icon: Icon, title, text }) => <article className="rounded-[23px_8px] border border-[#e8e0ef] bg-white p-[27px]" key={title}><Icon className="text-[#401b8c]" /><h3 className="mt-[38px] text-lg">{title}</h3><p className="mt-[9px] text-xs leading-[1.6] text-[#766d7c]">{text}</p></article>)}
        </div>
        <div className="my-[72px] grid grid-cols-[1.1fr_.9fr] items-center gap-[55px] rounded-[36px_10px] bg-[#f7f4fa] p-[clamp(30px,5vw,56px)] max-[1050px]:grid-cols-1">
          ...
          <div className="flex flex-wrap gap-[9px]">{cmsValues.commercialCapabilities.map((item) => <span className="rounded-full border border-[#ded3e8] bg-white px-[13px] py-2.5 text-xs font-semibold text-[#401b8c]" key={item}>{item}</span>)}</div>
        </div>
        <div className="border-t border-[#e8e0ef]">
          ...
          {cmsValues.projectControlModules.map((module, index) => (
