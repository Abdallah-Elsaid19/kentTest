...
export function FinalCtaSection() {
  ...
  const cmsValues = cms.resolve({ finalRoutes });
    ...
    <section className="relative overflow-hidden bg-primary py-[clamp(85px,10vw,145px)] text-white" id="final-cta">
      ...
      <div className={containerClass}>
        ...
        <div className="mt-[65px] grid grid-cols-3 border-y border-white/[.13] max-[780px]:grid-cols-1">
          {cmsValues.finalRoutes.map(({ icon: Icon, label, title, text, href }) => <a className="relative min-h-[250px] border-r border-white/[.13] p-[34px] text-white no-underline last:border-r-0 max-[780px]:min-h-0 max-[780px]:border-b max-[780px]:border-r-0 max-[780px]:last:border-b-0" href={href} key={title}><Icon className="text-[#f5c94f]" /><small className="mt-[31px] block text-[10px] font-bold uppercase tracking-[.15em] text-[#f5c94f]">{label}</small><h3 className="mt-2.5 text-[22px] !text-white">{title}</h3><p className="mt-[9px] text-xs leading-[1.65] text-white/50">{text}</p><ArrowUpRight className="absolute right-[29px] top-[31px] w-[19px]" /></a>)}
        </div>
