...
export function EligibilityCheckerSection() {
  ...
  const cmsValues = cms.resolve({ checkerQuestions });
  ...
  const chooseAnswer = (answer: string) => {
    ...
    setAnswers(next);
    if (question === cmsValues.checkerQuestions.length - 1) setCheckerComplete(true);
    ...
    <section className={softSectionClass} id="eligibility-checker">
      <div className={narrowContainerClass}>
        ...
        <div className="rounded-[38px_11px] border border-[#e8e0ef] bg-white p-[clamp(28px,5vw,52px)] shadow-[0_28px_80px_rgba(47,20,104,.08)]">
          {!checkerComplete ? (
            <>
              <div className="flex items-center gap-[7px]">{cmsValues.checkerQuestions.map((_, index) => <span key={index} className={`h-[5px] flex-1 rounded-full ${index <= question ? "bg-[#401b8c]" : "bg-[#e9e2ef]"}`} />)}<b className="ml-3 whitespace-nowrap text-[11px] text-[#766d7c]">{question + 1} / {cmsValues.checkerQuestions.length}</b></div>
              <div className="mt-11 flex items-center gap-[21px] max-[500px]:flex-col max-[500px]:items-start"><Compass className="h-[54px] w-[54px] rounded-[20px_7px] bg-[#f1eaf6] p-[14px] text-[#401b8c]" /><h3 className="text-[clamp(22px,3vw,33px)] leading-[1.2]">{cmsValues.checkerQuestions[question].text}</h3></div>
              <div className="mt-8 flex flex-wrap gap-[11px]">{cmsValues.checkerQuestions[question].answers.map((answer) => <button className="min-h-12 cursor-pointer rounded-lg border border-[#ded3e8] bg-[#f7f4fa] px-5 py-3 text-[13px] font-semibold text-[#401b8c] hover:border-[#401b8c] hover:bg-[#401b8c] hover:text-white max-[500px]:w-full" key={answer} onClick={() => chooseAnswer(answer)} type="button">{answer}</button>)}</div>
              <div className="mt-9 flex items-center justify-between border-t border-[#e8e0ef] pt-6"><button className="flex min-h-11 cursor-pointer items-center gap-[7px] border-0 bg-transparent text-[13px] font-semibold text-[#766d7c] disabled:cursor-default disabled:opacity-30" disabled={question === 0} onClick={() => setQuestion((value) => value - 1)} type="button"><ChevronLeft /> Back</button><span className="text-[11px] text-[#a298a8] max-[500px]:hidden">{cmsValues.checkerQuestions.length - question - 1} questions remaining</span></div>
            </>
          ) : (
