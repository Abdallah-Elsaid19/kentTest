...
export function EligibilitySection() {
  ...
  const cmsValues = cms.resolve({ eligibilityItems, eligibilityRequirements });
    ...
    <section className={`${sectionClass} isolate overflow-hidden`} id="eligibility">
      ...
      <div className={containerClass}>
        ...
        <div className="mt-10 border-t border-[#e8e0ef] pt-[31px]">
          ...
          <div className="flex flex-wrap gap-[11px]">{cmsValues.eligibilityRequirements.map((item) => <span className="rounded-full bg-[#f7f4fa] px-3 py-[9px] text-xs font-semibold text-[#2f1468]" key={item}>{item}</span>)}</div>
