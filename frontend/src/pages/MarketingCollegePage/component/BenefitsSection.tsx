...
export function BenefitsSection() {
  return (
    <section id="marketing-benefits" className={`${section} relative isolate overflow-hidden bg-[var(--color-soft)]`} aria-labelledby="marketing-benefits-title">
      <img
      ...
      <div className={`${shell} relative z-10`}>
        <FigmaSectionHeading id="marketing-benefits-title" eyebrow={benefitCopy.eyebrow} title={benefitCopy.title} description={benefitCopy.description} />
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {marketingWhyChooseUs.map(({ icon: Icon, title, desc }) => (
            <article key={title} className={`${card} group transition duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-[0_18px_45px_rgba(39,14,73,0.1)] motion-reduce:transform-none motion-reduce:transition-none`}>
        ...
        <dl className="mt-12 grid grid-cols-2 gap-6 rounded-2xl bg-white px-5 py-8 sm:px-8 lg:mt-16 lg:grid-cols-4">
          {marketingWhyChooseUsStats.map((stat) => (
