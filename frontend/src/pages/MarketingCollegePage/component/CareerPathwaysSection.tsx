...
export function CareerPathwaysSection() {
  return (
    <section id="marketing-outcomes" className={`${section} relative isolate overflow-hidden bg-primary-dark text-white`} aria-labelledby="marketing-careers-title">
      <img
      ...
      <div className={`${shell} relative z-10`}>
        <FigmaSectionHeading id="marketing-careers-title" eyebrow={careerCopy.eyebrow} title={careerCopy.title} description={careerCopy.description} tone="inverse" />
          ...
          {careerRoutes.map((route) => (
            <article key={route.label} className="rounded-2xl border border-white/15 bg-white/[.06] p-6 backdrop-blur-sm sm:p-7">
              ...
              <p className="mt-6 border-t border-white/15 pt-5 text-xs leading-6 text-white/55">
                Career outcomes: {route.outcomes.join(" · ")}
              </p>
            </article>
          ))}
        </div>
        <div className="mt-10 grid gap-x-8 sm:grid-cols-2 lg:grid-cols-3">
          {marketingOutcomes.map((outcome) => (
