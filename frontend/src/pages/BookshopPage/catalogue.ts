...
export function filterBooks(books: readonly Book[], query: string, category: BookCategory): readonly Book[] {
  const search = query.trim().toLocaleLowerCase("en-GB");
  return books.filter((book) => {
    const matchesCategory = category === "all" || book.categories.includes(category);
    const categoryNames = bookCategories.filter((item) => item.key !== "all" && book.categories.includes(item.key)).map((item) => item.label);
