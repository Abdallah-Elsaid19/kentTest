  ...
  return (
    <Dialog titleId={titleId} descriptionId={descriptionId} onClose={onClose} wide>
      <div className="mb-4 flex justify-end">
        <button type="button" onClick={onClose} aria-label="Close book details" className="grid size-11 place-items-center rounded-lg bg-kbc-purple-50 text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><X size={22} aria-hidden="true" /></button>
      </div>
      <div className="grid items-start gap-7 md:grid-cols-[.8fr_1.2fr] md:gap-10">
        ...
        <div className="min-w-0">
          ...
          <dl className="my-7 divide-y divide-kbc-purple-100 border-y border-kbc-purple-100">
            {bookMetadata.map(({ label, value }) => <div key={label} className="flex flex-wrap justify-between gap-3 py-4 text-sm"><dt className="text-[#756F79]">{label}</dt><dd className="font-semibold text-primary-dark">{value}</dd></div>)}
          </dl>
          <NavigationButton to={catalogue.requestHref} className="w-full gap-2">{catalogue.requestAction}<ArrowUpRight size={17} aria-hidden="true" /></NavigationButton>
        </div>
      </div>
    </Dialog>
  );
