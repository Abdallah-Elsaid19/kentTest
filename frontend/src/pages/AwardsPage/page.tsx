...
export default function AwardsPage() {
  return (
    <div className="kbc-figma-home bg-white font-body text-[#24152f] motion-reduce:[&_*]:!duration-[.01ms]">
      <RouteMeta fallbackTitle={awardsSeo.title} fallbackDescription={awardsSeo.description} seo={{ ...awardsSeo, canonical: `${environment.VITE_SITE_URL}/awards` }} />

      <CollegeHeroSurface image={awardsHero.image} titleId="awards-title" variant="funding" fullHeight patternSide="left">
        <div className={`${containerClass} pb-[clamp(70px,9vw,112px)] max-[780px]:pb-[70px]`}>
          <div className="mx-auto max-w-[920px] text-center">
            <p className={`${goldSectionEyebrowClass} !mx-auto !mb-8 !justify-center`}>{awardsHero.eyebrow}</p>
            <h1 id="awards-title" className="mt-0 text-4xl !font-medium !leading-[1.15] !tracking-tight !text-white sm:text-6xl lg:text-7xl">
              {awardsHero.title} <span className="text-[#F5C94F]">{awardsHero.accent}</span>
            </h1>
            <div className="mx-auto mt-[30px] max-w-[720px] space-y-4 text-base leading-[1.75] text-white/80 sm:text-lg">
              {awardsHero.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
            </div>
          </div>
        </div>
      </CollegeHeroSurface>
      ...
      <section className={`${sectionClass} !bg-[#f7f4fa]`} aria-labelledby="recognition-benefits-title">
        <div className={containerClass}>
          <SectionIntro id="recognition-benefits-title" eyebrow={recognitionBenefits.eyebrow} title={recognitionBenefits.title} align="left" />
          <div className="grid gap-5 md:grid-cols-3">
            {recognitionBenefits.items.map(({ icon: Icon, title, copy }) => (
      ...
      <section className={sectionClass} aria-labelledby="awards-cta-title">
        <div className={containerClass}>
          <CollegeCtaPanel id="awards-cta-title" eyebrow={awardsCta.eyebrow} title={awardsCta.title} description={awardsCta.description} actions={
            awardsCta.actions.map((action, index) => (
