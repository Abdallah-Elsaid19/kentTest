...
import { homeContract, homeValidators, parseHomeDocument } from "./schema";
...
function ContentList() {
  ...
  return <><div className="cms-page-heading"><div><p className="cms-eyebrow">Content library</p><h1>{params.get("collection") === "home" ? "Home Content" : "All content"}</h1><p>Find a section, refine your content, and publish when it’s ready.</p></div><a className="cms-button" href="/" target="_blank" rel="noreferrer">View website <ExternalLink size={16} /></a></div>
    <div className="cms-filters"><form onSubmit={event => { event.preventDefault(); updateFilter("search", search); }} className="cms-search"><Search size={18} /><input aria-label="Search content" placeholder="Search titles or content…" value={search} onChange={event => setSearch(event.target.value)} /><button type="submit">Search</button></form>
      ...
      <select aria-label="Filter by collection" value={params.get("collection") || ""} onChange={event => updateFilter("collection", event.target.value)}><option value="">All collections</option><option value="home">Home Content</option></select>
    </div>
    {query.isPending ? <LoadingState /> : query.error ? <ErrorState message={message(query.error)} /> : <>
      ...
      <div className="cms-content-grid">{query.data!.items.map(entry => <article className="cms-content-card" key={entry.key}><div className="cms-card-top"><span className="cms-section-icon">{entry.section === "hero" ? <Home /> : entry.section === "metadata" ? <Search /> : <Layers3 />}</span><Badge entry={entry} /></div><p className="cms-eyebrow">{entry.page} · Section {entry.sortOrder + 1}</p><h2>{entry.title}</h2><p className="cms-card-meta">Updated {dateLabel(entry.updatedAt)}<br />{entry.updatedByName || "Initial content import"} · Version {entry.version}</p><div className="cms-card-actions"><Link to={`/dashboard/content/${entry.key}?view=1`}><Eye size={16} />View</Link><Link to={`/dashboard/content/${entry.key}`}><Pencil size={16} />Edit content <ArrowRight size={15} /></Link></div></article>)}</div>
...
function EntryEditor({ entry }: { entry: ContentEntry }) {
  ...
  const submit = (action: ContentAction) => {
    ...
    if (action === "draft" || action === "publish") {
      const result = homeValidators[saved.section].safeParse(content);
  ...
  const schema = homeContract[entry.section];
  return <>
    <Link className="cms-back" to="/dashboard/content?collection=home"><ArrowLeft size={16} />Home Content</Link>
    <div className="cms-page-heading"><div><p className="cms-eyebrow">Home Content / {readOnly ? "View section" : "Edit section"}</p><h1>{entry.title}</h1><p>Version {saved.version} · Last saved {dateLabel(saved.updatedAt)}</p></div><Badge entry={saved} /></div>
...
function Preview() {
  const { key = "" } = useParams();
  const query = useQuery({ queryKey: ["cms", "preview", key], queryFn: async () => parseHomeDocument(await cmsRequest(`/entries/${encodeURIComponent(key)}/preview/`)), staleTime: 0 });
  return <><div className="cms-preview-bar"><Eye size={18} /><strong>Saved draft preview</strong><span>{key} · Other sections show published content</span><Link to={`/dashboard/content/${key}`}>Back to editor</Link></div>{query.isPending ? <LoadingState /> : query.error ? <ErrorState message={message(query.error)} /> : <HomeSections content={query.data!} preview />}</>;
}
